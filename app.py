...
2. **Explore Visualizations**: View the top 20 in-demand hard skills, tech stack, soft skills, industries, and benefits for the selected job title.
...
For any feedback or inquiries, please contact me at [support@techlist.me](mailto:support@techlist.me).
...
def replace_words_in_list(data, column_name, replacements):
    ...
    for i, items in data[column_name].items():
        ...
        if isinstance(items, list):
            data.at[i, column_name] = [replacements.get(item, item) for item in items]
...
if not filtered_data.empty:
    ...
    hard_skills_df = pd.DataFrame(
        hard_skills_freq.most_common(20), columns=["Skill", "Frequency"]
    )
    tech_stack_df = pd.DataFrame(
        tech_stack_freq.most_common(20), columns=["Tech", "Frequency"]
    )
    soft_skills_df = pd.DataFrame(
        soft_skills_freq.most_common(20), columns=["Skill", "Frequency"]
    )
    industries_df = pd.DataFrame(
        industries_freq.most_common(20), columns=["Industry", "Frequency"]
    )
    companies_df = pd.DataFrame(
        companies_freq.most_common(20), columns=["Company", "Frequency"]
    )
    benefits_df = pd.DataFrame(
        benefits_freq.most_common(20), columns=["Benefit", "Frequency"]
    )
    ...
    def plot_bar_chart(df, x, y, title):
        chart = (
            ...
            .mark_bar()
            .encode(
                x=alt.X(x, sort=alt.SortField(field="Frequency", order="descending")),
                y=y,
    ...
    st.altair_chart(
        plot_bar_chart(
            ...
            "Tech",
            "Frequency",
            f"Top 20 In-Demand Tech Stack for {keyword}",
        )
    )
    ...
    st.altair_chart(
        plot_bar_chart(
            ...
            "Skill",
            "Frequency",
            f"Top 20 In-Demand Hard Skills for {keyword}",
        )
    )
    ...
    st.altair_chart(
        plot_bar_chart(
            ...
            "Skill",
            "Frequency",
            f"Top 20 In-Demand Soft Skills for {keyword}",
        )
    )
    ...
    st.altair_chart(
        plot_bar_chart(
            ...
            "Industry",
            "Frequency",
            f"Top 20 In-Demand Industries for {keyword}",
        )
    )
    ...
    st.altair_chart(
        plot_bar_chart(
            companies_df, "Company", "Frequency", f"Companies Hiring Most for {keyword}"
        )
    )
    ...
    st.altair_chart(
        plot_bar_chart(
            benefits_df, "Benefit", "Frequency", f"Top 20 Benefits for {keyword}"
        )
    )
